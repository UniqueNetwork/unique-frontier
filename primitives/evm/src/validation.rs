...
impl<'config, E: From<InvalidEvmTransactionError>> CheckEvmTransaction<'config, E> {
	...
	pub fn with_account_or_sponsor_balance_for(&self, who: &Account, sponsor: Option<&Account>) -> Result<&Self, E> {
		let (_, effective_gas_price) = self.transaction_fee_input()?;
		let max_fee = effective_gas_price.unwrap_or_default().saturating_mul(self.transaction.gas_limit);
		let source_data = who;
		let value = self.transaction.value;

		if let Some(sponsor) = sponsor {
			#[cfg(feature = "debug-logging")]
			log::trace!(target: "sponsoring", "found sponsor: {:?}", sponsor);
			let sponsor_data = sponsor;
